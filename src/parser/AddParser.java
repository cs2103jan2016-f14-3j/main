...
public class AddParser extends ArgsParser {
	...
	private static final String INTERVAL_PERIOD_STRING_ANNUALLY = "annually";
	private static final String INTERVAL_PERIOD_STRING_YEARLY = "yearly";
	private static final String INTERVAL_PERIOD_STRING_FORTNIGHTLY = "fortnightly";
	private static final String INTERVAL_PERIOD_STRING_BIMONTHLY = "bimonthly";
	private static final String INTERVAL_PERIOD_STRING_MONTHLY = "monthly";
	private static final String INTERVAL_PERIOD_STRING_BIWEEKLY = "biweekly";
	private static final String INTERVAL_PERIOD_STRING_WEEKLY = "weekly";
	private static final String INTERVAL_PERIOD_STRING_DAILY = "daily";
	private static final String MESSAGE_EMPTY_ERROR = "Title cannot be empty!";
	private static final String MESSAGE_DATE_ERROR = "\"%s\" is not a valid date!";
	private static final String MESSAGE_BOUND_ERROR = "\"%s\" contains an invalid date!";
	private static final String MESSAGE_RECURRING_ERROR = "\"%s\" is not valid! Usage: \"r:<interval> until <bound date>";
	private static final String MESSAGE_EXCLUSION_ERROR = "\"%s\" is not valid! Usage: \"x:<start date> to <end date>\"";
	private static final String MESSAGE_PRIORITY_ERROR = "\"%s\" is not valid. Only high, medium or low is accepted!";
	private static final String MESSAGE_END_BEFORE_FROM = "End date cannot be before Start date!";
	private static final String MESSAGE_START_DATE_SPECIFIED = "To add a recurring task, the start date must be specified!";
	private static final String MESSAGE_DATES_SPECIFIED = "To add an event, both start and end dates must be specified!";
	private static final String MESSAGE_UNKNOWN = "Unknown error occured!";
		...
		for (int i = 0; i < delimiterIndexes.length; i++) {
			...
			if (isNonZeroDelimiterIndex(i)) {
				...
				case (TAG_PRIORITY):
					...
					break;
				case (TAG_EXCEPTION):
					setHasExclusion(true); // Flag to run recurring exclusion when set to true.
	...
	private void extractTitle(String commandArgumentsString) {
		...
		if (isEmptyArguments() || this.title.trim().equals("")) {
